...
def parse_val_unit(tup):
    if isinstance(tup, tuple): return tup[0], (tup[1] if len(tup)>1 else None)
    return tup, None

def make_spoofer(name):
    def spoof(*args, **kwargs):
        return {"type": name, "data": kwargs}
    return spoof
...
def process_kinetics(kinetics, fk_col, fk_id):
    ...
    row = KineticsData(
        ...
        A_val=A_v, A_unit=A_u, n=d.get("n"),
        Ea_val=Ea_v, Ea_unit=Ea_u, T0_val=T0_v, T0_unit=T0_u,
        w0_val=w0_v, w0_unit=w0_u, E0_val=E0_v, E0_unit=E0_u,
        Tmin_val=Tmin_v, Tmin_unit=Tmin_u, Tmax_val=Tmax_v, Tmax_unit=Tmax_u,
        comment=d.get("comment"),
        raw_data=repr(d) # Dump everything in case it's a Chebyshev or Troe object missing fields
    )
...
def load_kinetics_library(folder):
    ...
    try: 
        ...
        print(f"Error reading library {name}: {e}")
...
def load_kinetics_family(folder):
    ...
    if grp_file.exists():
        ...
        try: 
            ...
            print(f"Error reading family groups for {name}: {e}")
